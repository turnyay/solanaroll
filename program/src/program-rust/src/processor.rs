...
use crate::{
    ...
    instruction::SolanrollInstruction,
    state::Escrow,
    util::unpack_mint
};
...
impl Processor {
    pub fn process(
        ...
        let instruction = SolanrollInstruction::unpack(instruction_data)?;

        match instruction {
            SolanrollInstruction::CommitReveal { reveal_number, amount, roll_under } => {
                msg!("SolanrollInstruction: CommitReveal");
                Self::process_commit_reveal(accounts, amount, program_id)
            }
            SolanrollInstruction::ResolveRoll { reveal_number, amount, roll_under } => {
                msg!("SolanrollInstruction: ResolveRoll");
                Self::process_resolve_roll(accounts, amount, program_id)
            }
            SolanrollInstruction::Deposit { amount } => {
                msg!("SolanrollInstruction: Deposit");
                Self::process_deposit(accounts, amount, program_id)
            }
            SolanrollInstruction::Withdraw { amount } => {
                msg!("SolanrollInstruction: Withdraw");
